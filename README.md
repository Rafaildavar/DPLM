...
| Агент привязок | beta не содержит встроенного API-ключа и по умолчанию работает локально; пользователь может подключить свой Mistral, тогда очищенный запрос и контекст текущей задачи передаются провайдеру |
...
Публичная macOS beta не содержит общего `MISTRAL_API_KEY`: MAS использует
локальный детерминированный pipeline, пока пользователь сам не выберет Mistral в
`Настройки -> MAS и LLM`. Личный API-ключ сохраняется в системном Keychain и не
записывается в `config.json`, логи или release-артефакты. Mistral не нужен для
записи, обучения, распознавания и ручного создания привязок. Передаваемый агенту
...
По умолчанию MAS работает локально. Для облачного режима откройте
`Настройки -> MAS и LLM`, выберите `Mistral API`, укажите идентификатор модели и
свой API-ключ, затем нажмите `Сохранить` и `Проверить`. Ключ можно удалить там же.
