...
| Агент привязок | публичная beta не содержит API-ключа и работает локально; при собственной настройке Mistral очищенный текст запроса и контекст текущей задачи передаются провайдеру |
...
Публичная macOS beta не содержит `MISTRAL_API_KEY`: MAS использует локальный
детерминированный pipeline. Mistral и внешний web research не нужны для записи,
обучения, распознавания и ручного создания привязок и активируются только
отдельной конфигурацией собственной сборки или запуска из исходного кода.
