...
[Внедрение](#внедрение-и-интеграция) ·
...
## Внедрение и интеграция устройств

GestureBind использует камеру как входной сенсор, а распознанный жест превращает
в проверенное событие. Сегодня это событие управляет macOS; далее тот же слой
распознавания можно подключать к другим устройствам через адаптеры протоколов,
не меняя CV-модели и персональный датасет пользователя.

| Устройство или среда | Путь интеграции | Этап |
|---|---|---|
| MacBook / iMac и USB-камеры | локальное распознавание, hotkeys, media keys, запуск приложений, сценарии и управление курсором через Accessibility | поддерживается |
| Проекторы и внешние дисплеи | управление презентацией или медиаплеером на подключённом Mac; жест заменяет кликер или часть клавиатурных команд | поддерживается через macOS |
| Умные гаджеты и носимые устройства | BLE-адаптер для часов, браслетов, кнопок и контроллеров: жест может стать сигналом для выбранного действия гаджета | следующий этап |
| Умный дом и IoT | локальные адаптеры Matter, MQTT или HTTP передают только подтверждённое событие жеста в домашние сценарии: свет, медиа, климат или робототехника | следующий этап |
| Smart TV и медиаприставки | LAN-адаптеры производителей, HDMI-CEC через подключённый компьютер или companion-приложение для навигации и управления медиа | roadmap |
| Windows-компьютеры | нативная Windows-сборка, доступ к камере, системные hotkeys и pointer API; после этого появится Windows CI и установщик | активная разработка |
| VR/AR-гарнитуры | OpenXR-адаптер или плагин движка получает событие жеста для навигации, обучения и управления виртуальной сценой | roadmap |
| Смартфоны и планшеты | мобильный companion-клиент или web-контроллер для камеры, удалённого интерфейса и отображения статуса | roadmap |

**Текущий базовый путь:** установить приложение из DMG, выбрать встроенную или
USB-камеру, разрешить Camera, записать персональные жесты и привязать их к
безопасным действиям macOS. Для hotkeys и указателя пользователь отдельно
разрешает Accessibility. Жесты, камера и команды остаются на его устройстве.

Будущий слой адаптеров будет получать не видеопоток, а уже проверенное событие
вида «жест распознан, уверенность достаточна, действие разрешено». Это отделяет
персональное CV-ядро от конкретного устройства, сохраняет локальную обработку
камеры и даёт единый путь к умным гаджетам, телевизорам, Windows и VR/AR.
