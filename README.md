...
| Агент привязок | без API-ключа использует локальный fallback; при подключении Mistral очищенный текст запроса и контекст текущей задачи передаются провайдеру |
...
Mistral и внешний web research не нужны для записи, обучения, распознавания и
ручного создания привязок. Они активируются только отдельной конфигурацией.
Передаваемый агенту текст очищается от известных форматов ключей и токенов, но
пользователю всё равно не следует вводить в запросы пароли, персональные данные
или секретные рабочие сведения.
...
Опубликованный `v0.8.0` пока содержит только архив исходного кода. Готовое
приложение появится в GitHub Release после публикации `v0.8.1` в двух форматах:
`GestureBind-macos-v0.8.1.dmg` для обычной установки и резервный ZIP.
...
4. Откройте приложение из `Applications`. При первом запуске нажмите на него
   правой кнопкой и выберите `Open`.
5. Разрешите доступ к камере. Для hotkeys и управления курсором отдельно
...
До публикации `v0.8.1` release candidate можно запустить на macOS с Python
`3.11`. Инструкции для Windows появятся после завершения активной разработки и
приёмочного тестирования этой платформы.
