...
class GestureOnlineInfer:
    ...
    Работает в auto-hand режиме: детектор может видеть несколько рук, а
    обученная модель задаёт фактический формат признаков. ``set_two_hands``
    оставлен только для совместимости со старым UI.
    """
    ...
    def set_two_hands(self, enabled: bool) -> None:
        ...
        Сейчас инференс работает в auto-hand режиме: детектор может видеть
        несколько рук, а классификатор получает признаки по своей фактической
        размерности. Поэтому переключатель больше не пересоздаёт MediaPipe,
        а только сбрасывает сглаживающее окно.
        """
        target = bool(enabled)
