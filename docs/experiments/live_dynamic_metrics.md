...
- `Timeout` — `0` означает ждать жест без авто-пропуска; значение больше `0`
  включает автоматический `Missed` после заданного числа секунд;
- `Min conf` — минимальная confidence для зачета `Correct/Wrong`.

Основной протокол для ручной проверки: `Timeout = 0`, попытка заканчивается
только когда accepted prediction появился в live-пайплайне. Если жест явно не
распознался, пользователь нажимает `Пропуск`.
