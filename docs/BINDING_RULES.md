...
Документ фиксирует **контракт** между Flet UI привязки
(`app/flet_app/views/bindings.py`), сервисом исполнения
(`gesture_command_bridge.py`), исполнителем
...
группа в форме редактирования команды. В `action_spec` категория
**выводится** из значения
...
`commands.name` имеет UNIQUE-ограничение в БД (см. `docs/DB_SCHEMA.md`).
UI должен:
...
1. Открыть вкладку «Привязки» → «Создать команду».
...
6. Нажать «Сохранить» → `AppController.save_binding(...)`:
   - валидация (R1, R7, R8);
   - запись в БД;
